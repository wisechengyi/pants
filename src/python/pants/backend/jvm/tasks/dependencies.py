...
class Dependencies(ConsoleTask):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.is_internal_only = self.get_options().dependencies_is_internal_only
    self.is_external_only = self.get_options().dependencies_is_external_only
    if (self.is_internal_only and self.is_external_only):
      error_str = 'At most one of {internal} or {external} can be selected.'.format(
        internal=self.internal_only_flag, external=self.external_only_flag)
